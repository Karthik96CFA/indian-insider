...
State:  <engine>/.state/state.db (SQLite) — see ENGINE_HOME
Config: <engine>/.env
"""
...
def _engine_root() -> Path:
    """Monorepo: stocklens/engine. Legacy: ~/indian-insider. Override: ENGINE_HOME."""
    for key in ("ENGINE_HOME", "INDIAN_INSIDER_HOME"):
        val = os.environ.get(key, "").strip()
        if val:
            return Path(val).expanduser()
    monorepo = AGENTS_DIR.parent
    if (monorepo / "requirements.txt").exists() and (monorepo / "agents").is_dir():
        return monorepo
    legacy = Path.home() / "indian-insider"
    if legacy.is_dir():
        return legacy
    return monorepo
...
def get_gemini() -> "genai.Client":
    ...
    if not api_key:
        raise RuntimeError(
            f"GEMINI_API_KEY not set. Add it to {ENV_PATH}\n"
