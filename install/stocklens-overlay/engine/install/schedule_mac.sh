#!/usr/bin/env bash
# StockLens engine — full automation via launchd (times UTC; IST = UTC+5:30).
set -euo pipefail
...
write_plist() {
  ...
  local sched="$*"
  local label="in.market.stocklens.engine.${name}"
...
<plist version="1.0"><dict>
  ...
  <key>WorkingDirectory</key><string>${ENGINE_ROOT}</string>
  ...
  <key>EnvironmentVariables</key><dict>
    ...
    <key>ENGINE_HOME</key><string>${ENGINE_ROOT}</string>
  </dict>
  ${sched}
</dict></plist>
EOF
...
echo "Registering StockLens engine automation with launchd…"
